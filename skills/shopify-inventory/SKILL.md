...
- **`--location-name` is CASE-SENSITIVE.** `"warehouse se"` will NOT match `"Warehouse SE"`. If you're scripting this from a list of names with inconsistent casing, use `--location-id` instead, or look up the canonical name via `shopify-plugin:shopify-admin`.
